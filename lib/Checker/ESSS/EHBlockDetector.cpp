...
#include "Checker/ESSS/Interval.h"
#include "Checker/ESSS/ClOptForward.h"
#include "Checker/ESSS/FunctionVSA.h"
...
Summary EHBlockDetectorPass::summarizeBlock(const BasicBlock* currentBlock) const {
    ...
    for (const auto& instruction : *currentBlock) {
        if (auto CI = dyn_cast<CallInst>(&instruction)) {
            ...
            if (any_of(CI->users(), [](const User* user) {
                if (auto cmp = dyn_cast<CmpInst>(user)) {
                    for (auto cmpUser : cmp->users()) {
                        auto br = dyn_cast<BranchInst>(cmpUser);
            ...
            // Look for callees
            auto calleesIt = Ctx->Callees.find(CI);
            if (calleesIt != Ctx->Callees.end()) {
                // add the callee targets
                for (const auto* target : calleesIt->second) {
                    summary.ops.push_back({});
                    summary.ops.back().type = OperationType::Call;
                    summary.ops.back().callTargets = &calleesIt->second;
                }
            } else {
                CI->dump();
...
void EHBlockDetectorPass::identifyPotentialSanityChecks(const Function& function) {
    for (const auto& BB : function) {
        auto handle = [&](const AbstractCondition* cmp, pair<const Value*, unsigned int> value) {
            ...
            value->dump();
#endif
            if (auto CI = dyn_cast<CallInst>(cmp->getLhs())) {
                if (CI && (CI->getIntrinsicID() != Intrinsic::not_intrinsic || CI->isInlineAsm())) {
                    delete cmp;
                    return;
                }
            }
...
void EHBlockDetectorPass::storeData() {
...
#if 1
    for (auto& entry : functionToIntervalCounts) {
        const auto& pair = entry.first;
        auto& intervalCounts = entry.second;
    ...
    for (auto& entry : functionToIntervalCounts) {
        ...
        float sum = 0;
        for (const auto& entry2 : intervalCounts) {
            const auto& count = entry2.second;
        ...
        while (intervalCountsIt != intervalCountsEnd) {
            const auto& interval = intervalCountsIt->first;
            const auto& count = intervalCountsIt->second;
...
void EHBlockDetectorPass::stage0(Module* M) {
    ...
    for (const auto& F : *M) {
        ...
        if (testCases.hasValue()) {
            if (find(testCases.getValue(), F.getName()) == testCases.getValue().end()) {
                continue;
            }
        }
        ...
        for (const auto& entry : functionToSanityCheckCallAndCmpInstructionsIt->second) {
            const auto& value = entry.first;
            const auto& conditional = entry.second;
        ...
        for (const auto& entry : conditionalToErrorPath) {
        ...
        if (!blocksThatAreOnAtLeastOneInspectedPath.empty()) {
            for (const auto &BB: F) {
                ...
                for (const auto &instruction: BB) {
                    ...
                    if (calleesIt.hasValue()) {
                        for (const auto *target: calleesIt.getValue()->second) {
    ...
    if (ShowSafetyChecks) {
        ...
        for (const auto& item : safetyChecks) {
            const auto& safetyCheckComparison = item.first;
...
void EHBlockDetectorPass::processSafetyCheckMapping(const map<const AbstractComparison*, SafetyCheckData>& mapping) {
    ...
    for (const auto& item : mapping) {
        const auto& safetyCheckComparison = item.first;
        const auto& safetyCheckData = item.second;
        ...
        if (rhs.hasValue()) {
            ...
            expressionInterval.applyInequalityOperator(predicate, rhs.getValue());
            valueToInterval[make_pair(call, pair.second)].unionInPlace(expressionInterval);
        }
    }
    for (const auto& entry : valueToInterval) {
        const auto& pair = entry.first;
        const auto& interval = entry.second;
        if (Ctx->Layer1OnlyCalls.find(pair.first) != Ctx->Layer1OnlyCalls.end())
            continue;

        auto calleesIt = Ctx->Callees.find(pair.first);
        if (calleesIt != Ctx->Callees.end()) {
            for (const auto* target : calleesIt->second) {
    ...
    for (const auto* successor : successors(abstractComparisonBlock)) {
        ...
        for (const auto* block : blocks) {
            for (const auto &instruction: *block) {
                ...
                if (calleesIt.hasValue()) {
                    if (any_of(calleesIt.getValue()->second, [this](const Function *target) {
...
void EHBlockDetectorPass::stage1(Module* M) {
    ...
    for (const auto& F : *M) {
        ...
        for (const auto& item : it->second) {
            ...
            const auto& abstractCondition = item.second;
            ...
            valuePair.first->dump();
            if (auto I = dyn_cast<Instruction>(valuePair.first)) {
...
llvm::Optional<Interval> EHBlockDetectorPass::addForSpanAndReturnInstruction(PathSpan pathSpan, const ReturnInst* returnInstruction) {
    PHISet phiSet;
    ...
    if (returnValueConstant.hasValue()) {
        Interval tmp(true);
        tmp.appendUnsafeBecauseExpectsSortMaintained(
                Range(returnValueConstant.getValue(), returnValueConstant.getValue()));
        return {std::move(tmp)};
    } else {
        return {};
    }
...
llvm::Optional<bool> EHBlockDetectorPass::determineErrorBranchOfCallWithCompare(ICmpInst::Predicate predicate, unsigned int returnValueIndex, int rhs, const CallInst* checkedCall) {
    ...
    if (!calleesIt.hasValue()) return {};
    llvm::Optional<Interval> accumulation;
    for (const auto *callee: calleesIt.getValue()->second) {
        ...
        if (accumulation.hasValue())
            accumulation.getValue().intersectionInPlace(*maybeInterval.getValue());
        else
            accumulation = **maybeInterval;
    }

    // Only continue if we have a non-empty interval
    if (!accumulation.hasValue() || accumulation.getValue().empty()) return {};

    Interval predicateInterval(false);
    predicateInterval.applyInequalityOperator(predicate, rhs);
    predicateInterval.intersectionInPlace(accumulation.getValue());
...
void EHBlockDetectorPass::propagateCheckedErrors() {
    ...
    while (!functionsThatMightGetAPropagation.empty()) {
        ...
        for (const auto& item : potentialChecks) {
            const auto& abstractCondition = item.second;
        ...
        for (const auto& item : potentialChecks) {
            ...
            auto checkedValue = valuePair.first;
            auto returnValueIndex = valuePair.second;
            ...
            if (!rhs.hasValue()) continue;

            llvm::Optional<bool> decision = determineErrorBranchOfCallWithCompare(abstractComparison->getPredicate(), returnValueIndex, rhs.getValue(), checkedCall);
            if (!decision.hasValue())
                continue;
            bool trueTakenBranchIsAnError = decision.getValue();
            ...
            auto br = dyn_cast<BranchInst>(abstractComparison->getParent()->getTerminator());
            if (br && br->isConditional()) {
                ...
                for (auto *path: allPaths) {
                    ...
                    if (result.hasValue()) {
                        newIntervals.intervalFor(functionKeyPair, true).unionInPlace(result.getValue());
                        added = true;
                    }
                    delete path;
                }
