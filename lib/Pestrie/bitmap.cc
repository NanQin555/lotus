...
/* BENH: Set a single bit in a bitmap, returning true if it changed.  */
bool
bitmap_set_bit_p (bitmap head, int bit)
{
  bool r = true;
  ...
  if (ptr == 0)
    ...
    {
      r = !((ptr->bits[word_num] >> bit_num)&0x1);
      ptr->bits[word_num] |= bit_val;
    }
...
/* BENH */
bool bitmap_singleton_p(bitmap a)
{
  unsigned c = 0;
  bitmap_element *elt = a->first;

  if (elt == 0 || elt->next != 0) { return false; }

  for (unsigned ix = 0; ix != BITMAP_ELEMENT_WORDS; ix++) {
#if GCC_VERSION >= 3400
    /* Note that popcountl matches BITMAP_WORD in type, so the actual size
       of BITMAP_WORD is not material.  */
    c += __builtin_popcountl (elt->bits[ix]);
#else
    c += bitmap_popcount (elt->bits[ix]);
#endif
  }
...
/* BENH */
bool bitmap_multi_p(bitmap a) {
  unsigned c = 0;

  bitmap_element *elt = a->first;

  if (elt == 0) { return false; }
  if (elt->next != 0) { return true; }
...
#if GCC_VERSION >= 3004
  assert (sizeof(long) == sizeof (word));
...
bitmap_and_into (bitmap a, bitmap b)
{
  ...
  assert (!a->current == !a->first);
  assert (!a->current || a->indx == a->current->indx);
}
...
bitmap_equal_p (bitmap a, bitmap b)
{
  ...
  // We should consider two empty bitmaps are equal
  // Fixed by xiao
  a_elt = a->first;
  b_elt = b->first;
  if ( a_elt == NULL && 
       b_elt == NULL ) return true;
...
// Determine if two bitmaps have least one same bit.
int
bitmap_same_bit_p(bitmap a, bitmap b)
{
  bitmap_element *a_elt = a->first;
  bitmap_element *b_elt = b->first;
...
// Determine if all bits set in b are also set in a
int
bitmap_contain_p(bitmap a, bitmap b)
{
  bitmap_element *a_elt = a->first;
  bitmap_element *b_elt = b->first;
  ...
  while (a_elt && b_elt)
    {
      if (a_elt->indx < b_elt->indx)
	a_elt = a_elt->next;
      else if (b_elt->indx < a_elt->indx) return 0;
      else
	{
	  /* Matching elts, generate A & B.  */
	  unsigned ix;
...
// We output the contents of bitmap to the screen.
// It is always called in gdb for debugging.
void 
print_bitmap(bitmap a)
{
  bitmap_element *a_elt = a->first;
  unsigned ix;

  while (a_elt) {
    unsigned start = a_elt->indx * BITMAP_ELEMENT_ALL_BITS;
    for (ix = 0; ix < BITMAP_ELEMENT_WORDS; start += BITMAP_WORD_BITS, ix++) {
      BITMAP_WORD r = a_elt->bits[ix];
      while ( r ) {
	printf( "%d ", __builtin_ctz(r) + start ); 
	r -= r & -r;
      }
    }
...
static bitmap_element* 
recursive_construct_tree( bitmap_element* a )
{
  int i;
  int cnt_blocks = 0;
  bitmap_element *p;
  ...
  p = a;
  for ( i = (cnt_blocks-1) / 2; i > 0; --i, p = p -> next );
  if ( p->prev ) {
    p -> prev -> next = NULL;
    p -> prev = recursive_construct_tree( a );
  }

  if ( p -> next ) {
    p -> next -> prev = NULL;
    p -> next = recursive_construct_tree( p -> next );
  }
  
  return p;
}

// We transform the linked list to a balanced tree for O(lgn) query.
// After the transformation, user cannot set new bits any more.
void 
bitmap_make_tree( bitmap a )
{
  if ( a -> first ) {
    a -> first = recursive_construct_tree( a -> first );
    a -> current = a -> first;
  }
}

// Query is performed on a tree
int 
bitmap_tree_query( bitmap a, int bit )
{
  bitmap_element *p;
  int block = bit / BITMAP_ELEMENT_ALL_BITS;
  int bit_num, word_num;
  
  if ( a -> first == NULL ) return 0;
  if ( block == a -> current -> indx ) {
    bit_num = bit % BITMAP_WORD_BITS;
    word_num = bit / BITMAP_WORD_BITS % BITMAP_ELEMENT_WORDS;
    return ( a -> current -> bits[word_num] >> bit_num) & 1;
  }
  ...
  while ( p ) {
    if ( p -> indx == block ) {
      ...
      word_num = bit / BITMAP_WORD_BITS % BITMAP_ELEMENT_WORDS;
      a -> current = p;
      return (p->bits[word_num] >> bit_num) & 1;
    }

    if ( p -> indx < block ) p = p -> next;
    else p = p -> prev;
  }
...
static unsigned long 
recursive_count_bits( bitmap_element* p )
{
  int ix;
  unsigned long count = 0;
  ...
  for (ix = 0; ix != BITMAP_ELEMENT_WORDS; ix++) {
    ...
    /* Note that popcountl matches BITMAP_WORD in type, so the actual size
       of BITMAP_WORD is not material.  */
    count += __builtin_popcountl (p->bits[ix]);
#else
    count += bitmap_popcount (p->bits[ix]);
#endif
  }
  
  if ( p -> prev )
    count += recursive_count_bits( p -> prev );
  
  if ( p -> next )
    count += recursive_count_bits( p -> next );
  
  return count;
}

unsigned long 
bitmap_tree_count_bits (bitmap a)
{
  if ( a -> first )
    return recursive_count_bits( a -> first );
  
  return 0;
}

/*
 * The memory is only counting the index number and content of a block.
 * The returned value is bytes.
 */
double 
bitmap_calculate_memory( bitmap *ar, int size )
{
  int i;
  double mem;
  bitmap_element *a_elt;
  ...
  for ( i = 0; i < size; ++i ) {
    ...
    while ( a_elt ) {
      // indx + array of bits
      mem += sizeof(int) + BITMAP_ELEMENT_WORDS * sizeof(BITMAP_WORD);
      a_elt = a_elt -> next;
    }
  }
...
// The length of each label is defined outside
int 
bitmap_calculate_labels( bitmap *ar, int size )
{
  int i;
  int ans;
  bitmap_element *p;
  ...
  for ( i = 0; i < size; ++i ) {
    if ( ar[i] == NULL ) continue;
    // output number of bits for this bitmap
    ans++;
    // outout all elements in integer form
    ans += bitmap_count_bits( ar[i] );
  }

  return ans;
}

// Serialize a bitmap to a binary file
void 
bitmap_write_out( bitmap pb, FILE* fout, int fmt )
{
  int count;
  
  if ( fmt == COMPRESSED_FORMAT ) {
    // We directly output the bitmap blocks
    count = 0;
    for ( bitmap_element* elt = pb->first; elt; elt = elt->next)
      ++count;
    
    fwrite( &count, sizeof(int), 1, fout );
    
    for ( bitmap_element* elt = pb->first; elt; elt = elt->next) {
      fwrite( &(elt->indx), sizeof(unsigned), 1, fout );
      fwrite( elt->bits, sizeof(BITMAP_WORD), BITMAP_ELEMENT_WORDS, fout );
    }
  }
  else {
    // We output the indices where the bit is 1
    // It is suitable for extremely sparse bitmap
    count = bitmap_count_bits( pb );
    fwrite( &count, sizeof(int), 1, fout );
    
    bitmap_iterator bi;
    unsigned v;

    EXECUTE_IF_SET_IN_BITMAP( pb, 0, v, bi ) {
      fwrite( &v, sizeof(unsigned), 1, fout );
    }
  }
}

bitmap
bitmap_read_row( FILE* fp, int fmt, bool skip )
{
  int count;
  bitmap pb = NULL;
  int idx;

  if ( !skip ) pb = BITMAP_ALLOC(NULL);

  fread( &count, sizeof(int), 1, fp );

  if ( fmt == COMPRESSED_FORMAT ) {
    if ( skip ) {
      int skip_size = count * (sizeof(int) + sizeof(BITMAP_WORD) * BITMAP_ELEMENT_WORDS);
      fseek( fp, skip_size, SEEK_CUR );
    }
    else {
      for ( int i = 0; i < count; ++i ) {
	bitmap_element *ptr = bitmap_element_allocate (pb);
	fread( &idx, sizeof(int), 1, fp );
	ptr->indx = idx;
	fread( ptr->bits, sizeof(BITMAP_WORD), BITMAP_ELEMENT_WORDS, fp ); 
	bitmap_element_link (pb, ptr);
      }
    }
  }
  else {
